---
name: encode-httpx-repo-validate-recover
description: Test-time repo skill distilled from evaluator-approved benchmark evidence for encode__httpx.
active: true
quality_score: 0.74
quality_tier: repo
risk_flags: []
...
# encode-httpx-repo-validate-recover
...
- Support: `3`
- Support repos: encode__httpx

## Trigger

current task is in repo encode__httpx; focused validation resembles repeated test commands; failure signature matches repeated repo failures
...
3 task events; 0 positive events; paths=none; tests=python -m pytest -q 2>&1 | tail -20; failures=no-diff-recovery
