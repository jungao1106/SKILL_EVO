...
available to the writer or evaluator for skill promotion. We therefore evaluate
online test-time evolution with a subset-composition protocol: after each
verifier-scored gate, only the remaining unresolved tasks are retried with the
newly promoted session skills, and the final score is the union of all tasks
solved by any accepted gate. We describe a strict no-exact-transfer protocol and
an online test-time evolution protocol over a $2{\times}2$ executor matrix:
GLM-5.2 and GPT-5.5, each run in Pi and Claude-style agent harnesses. Current
GLM-5.2/Pi pilot results improve from 368/500 without skills to 423/500 after
three online gates; the full transfer matrix remains in progress.
\end{abstract}
...
to update skills during candidate generation or promotion. The official verifier
is invoked only to score completed gate attempts and to report resolved count
and resolved rate. Its labels are written to evaluation reports, not passed back
to the writer or evaluator.
...
\subsection{Downstream Test-Time Evolution Protocol}

The downstream online setting is intentionally not a repeated full-benchmark
rerun. Let $U_0$ be the SWE-Bench Verified task set and let $A_g$ be the set of
tasks solved by gate $g$. Gate 0 or gate 1 may be evaluated on the full task
set to establish the current accepted state. For each later gate, the executor
runs only on the unresolved subset
\[
U_g = U_{g-1} \setminus A_g.
\]
The reported cumulative score after gate $g$ is
\[
\left|\bigcup_{i \le g} A_i\right| / |U_0|.
\]
Thus a later gate cannot lower the score by failing an already solved task,
because those tasks are not rerun; it can only recover tasks that remain
unsolved. This matches the intended use of test-time skill evolution as a
verifier-free recovery process over the current error set.

Within each gate, the writer summarizes only public traces from the previous
unresolved subset. The evaluator promotes repo and failure-mode candidates using
its frozen SWEGym-trained policy and public evidence features. Official
SWE-Bench verifier outputs are used only after the gate run to report which
subset tasks were recovered and to construct the next unresolved subset. They
are not supplied as candidate inputs and do not update the persistent writer or
evaluator policies.
...
\begin{table}[h]
\caption{Primary SWE-Bench Verified within-cell results. GLM-5.2/Pi
numbers are current pilot results; the remaining cells are pending full runs.}
...
\midrule
GLM-5.2 / Pi / No Skills & 368/500 & 73.6\% & -- & -- & 2 \\
GLM-5.2 / Pi / Frozen SWEGym Library & 376/500 & 75.2\% & +8 & \tbd & 8 \\
GLM-5.2 / Pi / \method{} Gate 1 Full Eval & 379/500 & 75.8\% & +11 & \tbd & 5 \\
GLM-5.2 / Pi / \method{} Gate 2 Subset-Composed & 410/500 & 82.0\% & +42 & 0 by protocol & 6 \\
GLM-5.2 / Pi / \method{} Gate 3 Subset-Composed & 423/500 & 84.6\% & +55 & 0 by protocol & 8 \\
...
\paragraph{Current GLM-5.2/Pi pilot.}
The no-skill baseline solves 368/500 SWE-Bench Verified tasks. A frozen
SWEGym-derived library solves 376/500 under the same model, harness, task order,
and sandbox settings. The first online gate, evaluated on the full benchmark for
state initialization, solves 379/500. The second online gate is evaluated only
on the 121 tasks unresolved by gate 1; it recovers 31 of them, yielding a
subset-composed score of 410/500. The third gate is evaluated only on the
remaining 90 unresolved tasks; it recovers 13 additional tasks, yielding a
subset-composed score of 423/500. The eight-gate continuation is still running,
starting gate 4 on the remaining 77 unresolved tasks. The earlier full-500
rerun of gate 2 is not used for this metric because it violates the
subset-composition protocol.
...
Fifth, subset-composition evaluates recovery on the remaining error set and is
therefore not comparable to rerunning every gate on the full benchmark; we
report full-rerun numbers only as diagnostics. Finally, the full $2{\times}2$
transfer matrix is a planned setting. Within-cell
...
\texttt{evaluator\_policy.md}. Test-time evolution artifacts are stored under
\texttt{run\_logs/swebench\_verified\_tts\_evo/.../subset\_eval/};
\texttt{loop\_state.json} records the latest gate, cumulative resolved count,
subset recoveries, and remaining unresolved count.
...
# Subset-composition test-time skill evolution, gates 2..8.
python scripts/run_swebench_tts_subset_evo_loop.py \
  --run-id swebench_verified_tts_evo_from_direct_failures_20260704 \
  --start-gate 2 \
  --max-gate 8 \
  --concurrency 15
\end{verbatim}
